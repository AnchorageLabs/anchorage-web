...
export function HeroVisual() {
  return (
    <div className="relative min-h-[520px] lg:min-h-[660px]">
      <div className="animate-float-paper absolute left-8 top-16 h-[29rem] w-[24rem] rotate-[-2deg] bg-[var(--paper)] shadow-[18px_18px_0_rgba(17,17,17,0.16)] lg:left-16" />
      <div className="animate-float-paper absolute right-6 top-10 h-[26rem] w-[21rem] rotate-[2.5deg] bg-[var(--paper-soft)] shadow-[14px_14px_0_rgba(17,17,17,0.12)]" />
      ...
      <div className="animate-rotate-dashed-circle absolute left-1/2 top-[47%] h-[22rem] w-[22rem] -translate-x-1/2 -translate-y-1/2 rotate-[18deg] rounded-full border-2 border-dashed border-ink/60" />
      <div className="animate-rotate-dashed-circle absolute left-1/2 top-[47%] h-[30rem] w-[30rem] -translate-x-1/2 -translate-y-1/2 rotate-[-12deg] rounded-full border-2 border-dashed border-ink/35" />
      <svg className="animate-pulse-diagram absolute inset-0 h-full w-full text-ink" viewBox="0 0 600 660" aria-hidden="true">
      ...
      {nodes.map((node, index) => (
        <div key={node.label} className="animate-float-labels absolute rotate-[-2deg] rounded-2xl border-2 border-ink bg-[var(--paper)] px-4 py-3 font-mono text-xs font-black text-ink shadow-sketch" style={{ left: node.x, top: node.y, animationDelay: `${index * 0.15}s` }}>
